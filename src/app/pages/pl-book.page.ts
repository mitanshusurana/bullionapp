    ...
    <main class="max-w-md mx-auto px-4 py-4 pb-24">
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div>
          ...
          <input type="date" [value]="from()" (change)="from.set(($event.target as HTMLInputElement).value)" class="w-full rounded-lg border border-slate-300 bg-white px-2 py-1 text-sm" />
        </div>
        <div>
          <label class="block text-xs text-slate-600 mb-1">To</label>
          <input type="date" [value]="to()" (change)="to.set(($event.target as HTMLInputElement).value)" class="w-full rounded-lg border border-slate-300 bg-white px-2 py-1 text-sm" />
        </div>
      </div>
      <div class="mb-4">
