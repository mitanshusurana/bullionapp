      ...
      <main class="max-w-md mx-auto px-4 py-4 pb-24">
        <section class="grid grid-cols-2 gap-3" *ngIf="data() as d">
          <div class="rounded-xl bg-white shadow-soft p-4">
            ...
            <p class="mt-1 text-xl font-semibold">
              {{ d.totals.sale | currency: "INR" : "symbol" : "1.0-0" }}
            </p>
          </div>
          <div class="rounded-xl bg-white shadow-soft p-4">
            <p class="text-xs text-slate-500">Purchases</p>
            <p class="mt-1 text-xl font-semibold">
              {{ d.totals.purchase | currency: "INR" : "symbol" : "1.0-0" }}
            </p>
          </div>
          <div class="rounded-xl bg-white shadow-soft p-4">
            <p class="text-xs text-slate-500">Cash In</p>
            <p class="mt-1 text-xl font-semibold">
              {{ d.totals.cashin | currency: "INR" : "symbol" : "1.0-0" }}
            </p>
          </div>
          <div class="rounded-xl bg-white shadow-soft p-4">
            <p class="text-xs text-slate-500">Cash Out</p>
            <p class="mt-1 text-xl font-semibold">
          ...
          <div class="rounded-xl bg-white shadow-soft p-4 col-span-2">
            ...
            <p
              ...
              [class.text-emerald-600]="d.totals.net >= 0"
              [class.text-rose-600]="d.totals.net < 0"
            >
              {{ d.totals.net | currency: "INR" : "symbol" : "1.0-0" }}
            </p>
          </div>
        </section>
        ...
        <section class="mt-4">
          ...
          <div class="space-y-2" *ngIf="filteredEntries().length; else empty">
            <div
              *ngFor="let t of filteredEntries()"
...
export class DaybookPageComponent {
  ...
  private updateFilteredEntries() {
    ...
    const filtered = entries.filter((entry) => {
      ...
      if (query) {
        const searchable = [entry.name || "", entry.note || "", entry.type]
          .join(" ")
          .toLowerCase();
