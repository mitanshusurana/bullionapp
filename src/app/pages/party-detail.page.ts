      ...
      >
        <div class="max-w-md mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <a
            ...
            [queryParams]="{ name: party()?.name }"
            class="text-sm text-brand-700 underline underline-offset-4"
            >New Tx</a
          >
        </div>
      </header>
      ...
      <main class="max-w-md mx-auto px-4 py-4 pb-24">
        <section class="grid grid-cols-2 gap-3">
          <div class="rounded-xl bg-white shadow-soft p-4">
            ...
            <p class="mt-1 text-xl font-semibold">
              {{
                party()?.cashBalance || 0 | currency: "INR" : "symbol" : "1.0-0"
              }}
            </p>
          </div>
          <div class="rounded-xl bg-white shadow-soft p-4">
            ...
            <p class="mt-1 text-xl font-semibold">
              {{ party()?.metalBalance || 0 | number: "1.3-3" }} g
            </p>
          </div>
        </section>
        ...
        <section class="mt-6">
          ...
          <div
            class="space-y-2"
            *ngIf="
              getFilteredTransactions(loadedTransactions())?.length;
              else empty
            "
          >
            <div
              *ngFor="let t of getFilteredTransactions(loadedTransactions())"
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-slate-500 truncate">
                  {{
                    t.note ||
                      (t.type === "metalin" || t.type === "metalout"
                        ? (t.netWt || t.grossWt || 0) + " g"
                        : "")
                  }}
                </p>
              </div>
          ...
          <div
            *ngIf="
              hasMore() && getFilteredTransactions(loadedTransactions())?.length
            "
...
export class PartyDetailPageComponent {
  ...
  readonly party = computed(() =>
    this.parties
      .parties()
      .find((p) => p.name === this.route.snapshot.paramMap.get("id")),
  );
  ...
  private loadTransactions() {
    ...
    this.txSvc
      .fetchByParty(name, this.currentPage(), this.pageSize())
      .subscribe({
        next: (txs) => {
          const next =
            this.currentPage() === 0
              ? txs
              : [...this.loadedTransactions(), ...txs];
